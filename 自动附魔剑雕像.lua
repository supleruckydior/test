...
local waitingForRefresh = false  -- 是否正在等待刷新后的数据
...
local function processDataAndContinue(data)
    ...
    -- 如果正在处理，忽略这次数据（防止重复处理）
    if isProcessing then
        return
    end
    
    -- 如果正在等待刷新后的数据，标记为已收到
    if waitingForRefresh then
    ...
    if satisfiedCount < MAX_ENTRIES then
        ...
        if currentTokens >= nextCost then
            print("[继续] 等待后继续刷新...")
            task.wait(0.5)
            if refreshEnchant(lockTable) then
                waitingForRefresh = true  -- 标记正在等待刷新后的数据
            end
        else
            print("✗ 代币不足，停止运行")
            isRunning = false
...
syncEvent.OnClientEvent:Connect(function(data)
    ...
    elseif waitingForRefresh then
        -- 如果正在等待刷新后的数据，处理这次数据
        processDataAndContinue(data)
    else
        -- 如果不在等待刷新，忽略这次数据（可能是其他原因触发的同步）
        -- 不处理，避免重复运行
    end
end)
