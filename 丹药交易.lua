...
if isMobile then
    ...
    container.Size = UDim2.new(1, 0, 0, 800)
...
-- 如果是手机设备，调整滚动框内容大小
if isMobile then
    local totalHeight = 50 + 5*95 + 70 + 100 + 20
    contentFrame.Size = UDim2.new(1, 0, 0, totalHeight)
    contentFrame.Parent.CanvasSize = UDim2.new(0, 0, 0, totalHeight)
end

-- 获取远程事件
local elixirSyncEvent = ReplicatedStorage:WaitForChild("药水"):WaitForChild("同步模块"):WaitForChild("同步控制器"):WaitForChild("数据更新事件")
local addTradeItemEvent = ReplicatedStorage:WaitForChild("药水"):WaitForChild("功能"):WaitForChild("交易"):WaitForChild("新增交易物品")

-- 丹药数据存储
local elixirData = nil

-- 关键修复：确保数据同步处理在刷新前设置好
elixirSyncEvent.Event:Connect(function(data)
    elixirData = data
    statusLabel.Text = "数据已同步 "..os.date("%H:%M:%S")
end)
...
local function calculateElixirPoints()
    ...
    local backpack = elixirData["背包"] or {}
    if #backpack == 0 then
        warn("背包中没有物品")
        return {}, 0
    end
    
    local typeTotals = {0, 0, 0, 0, 0}
    local grandTotal = 0
    
    for _, item in ipairs(backpack) do
        if item and type(item) == "table" then
            local count = tonumber(item["数量"]) or 0
            local elixirType = tonumber(item["类型"]) or 0
            local quality = tonumber(item["品质"]) or 0
            
            if elixirType >= 1 and elixirType <= 5 and qualityPoints[quality] and count > 0 then
...
tradeButton.MouseButton1Click:Connect(function()
    ...
    -- 检查是否有丹药数据
    if not elixirData or not elixirData["背包"] then
...
refreshButton.MouseButton1Click:Connect(function()
    ...
    -- 触发数据同步
    local syncSuccess = pcall(function()
        -- 这里添加触发同步的代码，根据实际游戏可能需要调用特定远程事件
        -- 例如: elixirSyncEvent:FireServer("request_sync")
    end)
    
    if not syncSuccess then
        statusLabel.Text = "错误: 同步请求失败"
        return
    end
    
    -- 等待数据更新，最多等待5秒
    local startTime = os.time()
    while not elixirData do
...
-- 初始状态
statusLabel.Text = "请点击刷新按钮获取丹药数据"
