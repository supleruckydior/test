-- 市场一键上架翅膀脚本
-- 功能：筛选品质为6、第三个属性数值低于阈值的翅膀并自动上架到市场
...
local THRESHOLD = 1.32  -- 阈值：第三个属性数值低于此值的翅膀将被上架
local SELL_PRICE = 3700  -- 上架价格
local DELAY_BETWEEN_SELLS = 0  -- 每次上架之间的延迟（秒）

-- 获取出售事件
local sellItemEvent = ReplicatedStorage:WaitForChild("\228\186\139\228\187\182"):WaitForChild("\229\133\172\231\148\168"):WaitForChild("\233\156\178\229\164\169\229\149\134\229\186\151"):WaitForChild("\232\174\190\231\189\174\231\137\169\229\147\129")
...
-- 上架翅膀的函数
local function sellWing(itemId)
    local success, err = pcall(function()
        local args = {
            "\232\163\133\229\164\135",  -- "装备"
            itemId,
            SELL_PRICE
        }
    ...
    if success then
        ...
        warn('上架失败，物品ID:', itemId, '错误:', err)
        return false
    end
end

-- 筛选并上架符合条件的翅膀
local function filterAndSellWings()
    ...
    print(string.format('  需要上架的翅膀: %d', #toSellWings))
    
    if #toSellWings == 0 then
        print('✓ 没有需要上架的翅膀')
        return
    end
    ...
    print('=' .. string.rep('=', 60))
    print('待上架翅膀列表：')
    for i, wing in ipairs(toSellWings) do
    ...
    -- 第二遍：执行上架操作
    print(string.format('开始上架 %d 个翅膀...', #toSellWings))
    
    local successCount = 0
    local failCount = 0
    
    for i, wing in ipairs(toSellWings) do
        print(string.format('[%d/%d] 正在上架 物品ID=%s (品质=%d, 数值=%.3f)...', 
            i, #toSellWings, tostring(wing.itemId), wing.quality, wing.attrValue))
        
        if sellWing(wing.itemId) then
        ...
        -- 延迟以避免请求过快
        if i < #toSellWings then
            task.wait(DELAY_BETWEEN_SELLS)
        end
    end
    
    print('=' .. string.rep('=', 60))
    print(string.format('上架完成！成功: %d, 失败: %d', successCount, failCount))
    print('=' .. string.rep('=', 60))
end
...
print(string.format('上架价格：%s', SELL_PRICE))
print('正在等待数据加载...')
print('=' .. string.rep('=', 60))
